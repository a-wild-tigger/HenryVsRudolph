...
/** This class implements a Hidden Markov Model, as well as
 ...
 Modified Baum Welch to Handle multiple sequences of observations
 @author Anil Vaitla (avaitla16@gmail.com)
 */
public class HMM implements Serializable {
    ...
    double epsilon = .0001;
    private boolean assert1Sum(double[] array) {
        double value = 0;
    ...
    public boolean validateModel() {
        boolean aValue = assert1Sum(pi);
        for(int i = 0; i!= numStates; i++) {
            aValue &= assert1Sum(a[i]);
            aValue &= assert1Sum(b[i]);
        }
    ...
    public void train(int[][] o, int steps) {
        int numObs = o.length;
        double[][][] fwds = new double[numObs][][];
        double[][][] bwds = new double[numObs][][];

        double[] pi2 = new double[numStates];
        double[][] a2 = new double[numStates][numStates];
        double[][] b2 = new double[numStates][sigmaSize];

        for (int s = 0; s < steps; s++) {
        /* calculation of Forward- und Backward Variables from the
	       current model */
            for(int obs = 0; obs != numObs; obs++) {
                fwds[obs] = forwardProc(o[obs]);
                bwds[obs] = backwardProc(o[obs]);
            }
        ...
        /* re-estimation of initial state probabilities */
            for (int i = 0; i < numStates; i++) {
                for(int obs = 0; obs < numObs; obs++)
                    pi2[i] += gamma(i, 0, o[obs], fwds[obs], bwds[obs]);
                pi2[i] = Math.min(Math.max(divide(pi2[i], numObs), 0), 1);
            }
            ...
            for (int i = 0; i < numStates; i++) {
                for (int j = 0; j < numStates; j++) {
                    ...
                    for(int obs = 0; obs < numObs; obs++) {
                        ...
                        int T = ob.length;
                        for (int t = 0; t <= T - 1; t++) {
                            num += p(t, i, j, ob, fwds[obs], bwds[obs]);
                            denom += gamma(i, t, ob, fwds[obs], bwds[obs]);
                        }
                    }
                    a2[i][j] = divide(num, denom);
                }
            }
            ...
            for (int i = 0; i < numStates; i++) {
                for (int k = 0; k < sigmaSize; k++) {
                    ...
                    for(int obs = 0; obs < numObs; obs++) {
                        ...
                        int T = ob.length;
                        for (int t = 0; t <= T - 1; t++) {
                            double g = gamma(i, t, ob, fwds[obs], bwds[obs]);
                            num += g * (k == ob[t] ? 1 : 0);
                            denom += g;
                        }
                    }
                    b2[i][k] = divide(num, denom);
                }
            }
